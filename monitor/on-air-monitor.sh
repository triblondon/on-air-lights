...
log stream --predicate 'sender contains "appleh13camerad" and (composedMessage contains "PowerOnCamera" or composedMessage contains "PowerOffCamera")' | grep --line-buffered "ISP_Power" | while read -r line; do
  if [[ $line == *"PowerOn;"* ]]; then
    ...
    curl -v "http://homeassistant.local:8123/api/states/$ha_entity" -H "Authorization: Bearer $ha_token" -H "Content-Type: application/json" -d '{"state": "on", "attributes":{"editable":true,"icon":"mdi:video","friendly_name":"Andrew Laptop camera"}}'
  else
    ...
    curl -v "http://homeassistant.local:8123/api/states/$ha_entity" -H "Authorization: Bearer $ha_token" -H "Content-Type: application/json" -d '{"state": "off", "attributes":{"editable":true,"icon":"mdi:video","friendly_name":"Andrew Laptop camera"}}'
  fi
done
